...
#define DEFAULT_LIB_NAME "ordered"
...
static const struct lib_def libs[] = { { .name = "ordered", .handler =
		exchng1d_ordered }, { .name = "blocking", .handler =
		exchng1d_blocking }, { .name = "combined", .handler =
...
static void usage(void) {
	...
	fprintf(stderr,
			"  --lib		set the exchange type to use "
					"\n\t\t[ ordered | blocking | combined | buffered | async ]\n");
